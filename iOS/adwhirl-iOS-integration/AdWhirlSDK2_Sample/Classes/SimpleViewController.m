...
/* PROOF ProofViewController */
#import "ProofViewController.h"
/* PROOF kSampleAppKey */
#define kSampleAppKey @"b6fdef68f9e045d4a245f028f71e0065"
...
- (void)performEvent {
  self.label.text = @"PE Event performed";
}
...
- (void)performEvent2:(AdWhirlView *)adWhirlView {
  ...
  replacement.textColor = [UIColor whiteColor];
  replacement.textAlignment = UITextAlignmentCenter;
  replacement.text = [NSString stringWithFormat:@"PE2 Event performed, view %x", adWhirlView];
  [adWhirlView replaceBannerViewWith:replacement];
  [replacement release];
  [self adjustAdSize];
  self.label.text = [NSString stringWithFormat:@"PE2a Event performed, view %x", adWhirlView];
}

/**
 * PROOF custom event
 */
- (void)proof:(AdWhirlView *)adWhirlView {
  //build up a webview for the 140 Proof Ad unit
  UIWebView *replacement = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, 320, 50)];
  [replacement setDelegate:self];

  //populate the webview
  //[replacement loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://api.140proof.com/acls/user.html?user_id=[TWITTER USER_ID]&app_id=[YOUR 140 PROOF APP_ID]&style=sban&width=320&height=50"]]];
  [replacement loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://api.140proof.com/acls/user.html?user_id=140ProofAds&app_id=1&style=sban&width=320&height=50"]]];

  //show the webview
  [adWhirlView replaceBannerViewWith:replacement];
  [replacement release];
  [self adjustAdSize];
  self.label.text = [NSString stringWithFormat:@"PROOF Event performed, view %x", adWhirlView];
}
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request
   ...
   if ( ( [ [ requestURL scheme ] isEqualToString: @"http" ] 
      ...
      //ProofViewController *proofView = [[ ProofViewController alloc ] autorelease];    
   ...
   if ( [ [ requestURL scheme ] isEqualToString: @"myapp" ] 
       && ( navigationType == UIWebViewNavigationTypeLinkClicked ) ) { 
